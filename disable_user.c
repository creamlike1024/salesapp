...
int disable_user_window();
int disable_user_callback(int rtv);
...
int disable_user(MYSQL * con,int executor)
{
    ...
    while(1)
    {
        rtv=disable_user_window();
        rtv=disable_user_callback(rtv);
        if(CMD_UNKNOWN==rtv)
        {
            unknown_cmd();
            continue;
        ...
        if(CMD_BY_ID==rtv)
        {
            ...
            char sql_enable_user[128]="UPDATE USERS SET STATUS=0 WHERE ID=";
            char user_id[64]="";
