...
int change_good_price_window();
int change_good_price_callback(int rtv);
...
int change_good_price(MYSQL * con)
{
    ...
    while(1)
    {
        rtv=change_good_price_window();
        rtv=change_good_price_callback(rtv);
        if(CMD_UNKNOWN==rtv)
        {
            unknown_cmd();
            continue;
        ...
        if(CMD_BY_CODE==rtv)
        {
            ...
            char sql_change_good_price[128]="UPDATE GOODS SET PRICE=";
            char price[128]="";
            printf("输入商品新售价\n");
            get_str(&price);
            char code_str[128]="";
