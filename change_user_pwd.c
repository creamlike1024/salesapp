...
int change_user_pwd(MYSQL * con,int executor)
{
    ...
    char sql_change_user_pwd[128]="UPDATE USERS SET PASSWORD='";
    strcat(sql_change_user_pwd,password);
    strcat(sql_change_user_pwd,"' WHERE ID=");
    char user_id[64]="";
    itoa(get_user_id(con,row_num),user_id,10);  //将get_user_id返回的整型 id 转为字符串
    strcat(sql_change_user_pwd,user_id);
    if(mysql_query(con,sql_change_user_pwd))
    {
        db_error(con);
    }
    printf("成功修改了密码\n");
